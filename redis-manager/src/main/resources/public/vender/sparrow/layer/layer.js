	...
	Class.pt.vessel = function(conType, callback){
		...
		callback([
			...
			'<div class="'+ doms[0] +' '+ (doms.anim[config.shift]||'') + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] +';border:0px;height:' + config.area[1] + (config.fix ? '' : ';position:absolute;') +'">'
	...
	Class.pt.tips = function(){
		...
		var layArea = [layero.outerWidth(), layero.outerHeight()], follow;
		//修改!!!by newe
		if ( 'string' === typeof config.follow )
		{
			follow = $( config.follow );
		}
		else
		{
			follow = newe.jq_dom( config.follow );
		}
